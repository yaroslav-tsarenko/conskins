...
function getSiteUrl(): string {
  const raw = process.env.NEXT_PUBLIC_SITE_URL || "https://conskins.com";
  return raw.replace(/\/+$/, "");
}
  ...
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    <div style="text-align:center;margin-bottom:24px;">
      <a href="${getSiteUrl()}" style="text-decoration:none;">
        <span style="font-size:24px;font-weight:900;color:${BRAND_COLOR};letter-spacing:-0.03em;">ConSkins</span>
      </a>
    </div>
    ...
    <div style="text-align:center;margin-top:24px;font-size:12px;color:#999;line-height:1.6;">
      <p style="margin:0 0 4px;">&copy; ${new Date().getFullYear()} ConSkins. All rights reserved.</p>
      <p style="margin:0;">CONDEO LTD &middot; Dept 6790, 196 High Road, Wood Green, London, N22 8HH &middot; Company number 17225871 &middot; <a href="${getSiteUrl()}" style="color:${BRAND_COLOR};text-decoration:none;">conskins.com</a></p>
...
export async function sendWelcomeEmail(email: string, name?: string | null): Promise<boolean> {
  ...
  return send({
    to: email,
    subject: "Welcome to ConSkins!",
    html: emailWrapper(
      `
      <h1 style="margin:0 0 16px;font-size:22px;font-weight:800;color:${TEXT_COLOR};">${greeting}, welcome to ConSkins!</h1>
      <p style="color:${MUTED_COLOR};line-height:1.6;margin:0 0 16px;">
        Your account is ready. Link your Steam account and start buying CS2 skins with fast, secure delivery.
      </p>
      <div style="background:#f9f9f9;border-radius:8px;padding:16px;margin:0 0 24px;">
        ...
        <ul style="margin:0;padding:0 0 0 20px;color:${MUTED_COLOR};line-height:1.8;">
          <li>Browse the store for the skins you want</li>
          <li>Link your Steam account and add your trade URL</li>
          <li>Buy skins delivered to your inventory, usually within minutes</li>
          <li>Track every purchase under My Trades</li>
        </ul>
      </div>
      ${button(`${siteUrl}/catalog`, "Browse skins")}
      <p style="color:#999;font-size:13px;margin:24px 0 0;text-align:center;">
        Buyer protection on every trade
      </p>
    `,
      { preheader: `${greeting}! Your ConSkins account is ready.` },
    ),
  });
}
  ...
  return send({
    ...
    html: emailWrapper(
      ...
      <table style="width:100%;border-collapse:collapse;margin-bottom:16px;">
        <tr>
          <td style="width:50%;vertical-align:top;padding-right:12px;">
            ...
            <p style="margin:0;font-size:13px;color:${TEXT_COLOR};line-height:1.55;">
              <strong>CONDEO LTD</strong><br />
              Company number: 17225871<br />
              Dept 6790, 196 High Road<br />
      ...
      <p style="color:#999;font-size:12px;margin:24px 0 0;line-height:1.6;text-align:center;">
        ...
        For any questions, reply to this email or contact <a href="mailto:info@conskins.com" style="color:${BRAND_COLOR};">info@conskins.com</a>.
      </p>
    `,
...
export async function sendPasswordResetEmail(email: string, resetUrl: string, name?: string | null): Promise<boolean> {
  ...
  return send({
    to: email,
    subject: "Reset your ConSkins password",
    html: emailWrapper(
      `
      <h1 style="margin:0 0 16px;font-size:22px;font-weight:800;color:${TEXT_COLOR};">Reset your password</h1>
      <p style="color:${MUTED_COLOR};line-height:1.6;margin:0 0 16px;">
        ${greeting}, we received a request to reset the password on your ConSkins account.
    ...
    `,
      { preheader: "Reset your ConSkins password — link expires in 1 hour." },
    ),
  });
}
...
export async function sendContactAutoReplyEmail(submission: ContactSubmission): Promise<boolean> {
  return send({
    to: submission.email,
    subject: "We received your message — ConSkins",
    html: emailWrapper(
      `
