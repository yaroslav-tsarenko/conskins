...
export function ContactBlock() {
  return (
    <div className="mt-2 rounded-[var(--radius-lg)] border border-[color:var(--color-border)] bg-[color:var(--color-bg-elevated)] px-4 py-3.5 text-sm leading-relaxed">
      <p>
        <strong>{brand.company.legalName}</strong>
        <br />
        Company number: {brand.company.number}
        <br />
        Registered office: {brand.company.addressLine}
        <br />
        Email:{" "}
        <a href={brand.contact.emailHref} className="text-[color:var(--color-accent)] hover:underline">
          {brand.contact.email}
        </a>
      </p>
    </div>
