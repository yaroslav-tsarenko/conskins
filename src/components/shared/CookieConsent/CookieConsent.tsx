...
const STORAGE_KEY = "conskins-cookie-consent-v1";
export const COOKIE_SETTINGS_EVENT = "conskins:cookie-settings";
...
function writeConsent(consent: StoredConsent) {
  try {
    ...
    window.dispatchEvent(new CustomEvent("conskins:consent-updated", { detail: consent }));
  } catch {
    /* noop */
  }
}
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="glass fixed inset-x-0 bottom-0 z-[70] border-t border-[color:var(--color-border)] shadow-[0_-24px_60px_-20px_rgba(0,0,0,0.65)]"
        >
          <div className="mx-auto w-full max-w-[var(--max-width)] px-4 py-4 sm:px-6 sm:py-5 lg:px-8">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <h2
                    id="ds-cookie-title"
                    className="font-display text-[15px] font-bold leading-tight text-[color:var(--color-text)]"
                  >
                    We use cookies
                  </h2>
                  <p className="mt-1 max-w-2xl text-[12.5px] leading-relaxed text-[color:var(--color-text-secondary)]">
                    Essential cookies keep sign-in and trades working. With your
                    permission we also use analytics to improve ConSkins.{" "}
                    <Link
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2">
                <button
                  ...
                  className="rounded-lg px-3 py-2.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[color:var(--color-text-tertiary)] transition hover:text-[color:var(--color-text)]"
                >
                ...
                {detailsOpen ? (
                  <button
                    type="button"
                    onClick={saveCustom}
                    className="flex-1 rounded-lg border border-[color:var(--color-border)] px-5 py-2.5 text-xs font-semibold text-[color:var(--color-text)] transition hover:border-[color:var(--color-primary)] sm:flex-none"
                  >
                    Save choices
                  </button>
                ) : (
                ...
                <button
                  type="button"
                  onClick={acceptAll}
                  className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-[color:var(--color-primary)] px-5 py-2.5 text-xs font-bold text-[color:var(--color-primary-fg)] shadow-[var(--shadow-glow-volt)] transition hover:brightness-110 sm:flex-none"
                >
            ...
            <AnimatePresence initial={false}>
              {detailsOpen && (
                ...
                >
                  <div className="mt-4 grid gap-1 rounded-xl border border-[color:var(--color-border)] bg-[color:var(--color-bg)] p-2 sm:grid-cols-3">
